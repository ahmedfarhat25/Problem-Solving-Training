...
void abufarhat()
{
    string s1,s2,s3,res="";cin>>s1>>s2>>s3;
    loop(sz(s3))
    {
        jloop(26)
        {
            if(isdigit(s3[i])){res.push_back(s3[i]); break;}
            else if (s3[i]==s1[j] or s3[i]==(s1[j]-32))
            {
                if(isupper(s3[i])){res.push_back(toupper(s2[j])); break;}
                else {res.push_back(s2[j]); break;}
            }
        }
    }
    cout << res el;
